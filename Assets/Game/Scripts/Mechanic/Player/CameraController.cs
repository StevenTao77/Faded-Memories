...
public class CameraController : MonoBehaviour
{
    ...
    [Tooltip("Keep this as Everything if you don't use custom layers.")]
    ...
    void HandleFreeAngleView()
    {
        ...
        Vector3 desiredPosition = lookTarget + rotation * dir;

         
        RaycastHit[] hits = Physics.SphereCastAll(lookTarget, cameraRadius, (desiredPosition - lookTarget).normalized, distance, collisionMask);
        ...
        {
            //Ignore the player object and any of its children
            if (hit.transform == target || hit.transform.IsChildOf(target))
                continue;

            // ignore triggers to prevent the camera from getting stuck on invisible colliders
            ...
            if (hit.distance < closestDistance && hit.distance > 0)
            {
                ...
                bestPosition = hit.point + hit.normal * cameraRadius;
            }
        }
